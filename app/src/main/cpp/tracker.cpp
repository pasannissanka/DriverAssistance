...
void Tracker::AssociateDetectionsToTrackers(const std::vector<BoxInfo> &detection,
                                            std::map<int, Track> &tracks,
                                            std::map<int, cv::Rect> &matched,
                                            std::vector<BoxInfo> &unmatched_det,
                                            float iou_threshold)
{
    ...
    for (size_t i = 0; i < detection.size(); i++)
    {
        ...
        {
            iou_matrix[i][j] = CalculateIou(detection[i].box, trk.second);
            j++;
        }
    }
    ...
    for (size_t i = 0; i < detection.size(); i++)
    {
        ...
        for (const auto &trk : tracks)
        {
            if (0 == association[i][j])
            {
                ...
                {
                    matched[trk.first] = detection[i].box;
                    matched_flag = true;
                }
...
void Tracker::Run(const std::vector<BoxInfo> &detections)
{
    ...
    {
        Track _tracker;
        _tracker.Init(det.box, det.label, det.score);
        // Create new track and generate new ID
        tracks_[id_++] = _tracker;
    }
